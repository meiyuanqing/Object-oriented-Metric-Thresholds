...
"""
Author : Yuanqing Mei
File: 5_threshold_validating.py
Datetime : 2022/3/30 16:12
Principles: high cohesion, low coupling
HomePage : https://github.com/meiyuanqing
Email : dg1533019@smail.nju.edu.cn

将元分析的阈值在每个项目版本的倒数第二个版本组成的验证集上，用GM>=0.6验证
Performance metric: AUC,precision,recall,F1, FPR
Thresholds derived from five supervised and their meta-analysis will be validated on 66 versions
in validating data sets.
...
def predictive_performance(metric, metric_p, metric_t, df_test):
    ...
    return precision_value, recall_value, auc_value, auc_value_variance, gm_value, f1_value, bpp_value, fpr, tnr_value


def thresholds_validate(working_dir, result_dir):
    import os
    import csv
    import pandas as pd
    from sklearn.metrics import recall_score, precision_score, f1_score, roc_curve, auc, roc_auc_score, confusion_matrix
    ...
    validating_dir = working_dir + "pyData\\data_defects_java_validtaing\\"
    ...
    performnce_columns = ['fileName', 'metric', 'Sample_size', 'threshold', 'threshold_stdError', 'precision', 'recall',
                          'auc', 'auc_var', 'gm', 'f1', 'bpp', 'fpr', 'tnr']
    universal_t_performance = pd.DataFrame(columns=performnce_columns)
    auc_t_performance = pd.DataFrame(columns=performnce_columns)
    bender_t_performance = pd.DataFrame(columns=performnce_columns)
    bpp_t_performance = pd.DataFrame(columns=performnce_columns)
    gm_t_performance = pd.DataFrame(columns=performnce_columns)
    mfm_t_performance = pd.DataFrame(columns=performnce_columns)

    for root, dirs, files in os.walk(validating_dir):
        for name in files:
            print(name)

            df_validate = pd.read_csv(validating_dir + name)
            # drop all rows that have any NaN values,删除表中含有任何NaN的行,并重新设置行号
            df_validate = df_validate.dropna(axis=0, how='any', inplace=False).reset_index(drop=True)

            for metric in metric_names:
                print(name, metric)

                df_validate = df_validate[~df_validate[metric].isin(['undef', 'undefined'])]
                # 由于bug中存储的是缺陷个数,转化为二进制存储,若x>2,则可预测bug为3个以上的阈值,其他类推
                df_validate['bugBinary'] = df_validate.bug.apply(lambda x: 1 if x > 0 else 0)

                if len(set(df_validate['bugBinary'].values)) == 1:
                    continue

                if len(df_validate[df_validate[metric] != 0]) < 6:
                    continue
                ...
                for line_t in lines_t:
                    ...
                    precision, recall, auc, auc_var, gm, f1, bpp, fpr, tnr = \
                        predictive_performance(metric, metric_p, metric_t, df_validate)
                    print(name, metric, file_t.split('_')[0], precision, recall, auc, auc_var, gm, f1, bpp, fpr, tnr)
                    if file_t.split('_')[0] == 'auc':
                        auc_t_performance = auc_t_performance.append(
                            {'fileName': name[:-4], 'metric': metric, 'Sample_size': len(df_validate),
...
if __name__ == '__main__':
    ...
    working_Directory = "F:\\Thesis\\"
    result_Directory = "F:\\Thesis\\staticThresholdsValidating\\"
    os.chdir(working_Directory)

    thresholds_validate(working_Directory, result_Directory)
